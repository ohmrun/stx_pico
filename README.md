...
```haxe
static public function flat_map<Ti,Tii,R>(self:Either<Ti,Tii>,fn:Tii->Either<Ti,R>
:Either```<Ti,R>
...
```haxe
static public function bind_fold<T,Ti,TT>(self:StdArray<T>,pure:Ti->TT,init:Ti,bind:TT-
(Ti->TT```)->TT,fold:T->Ti->Ti):TT
...
```haxe
static public function reduce<T,TT>(self:StdArray<T>,unit:Void->TT,pure:T->TT
plus:TT```->TT->TT):TT
...
```haxe
static public inline function partition<T>(self: StdArray<T>,f: T -> Bool): { a :
StdArra```y<T>, b :  StdArray<T> }
